...
export default function RegisterPage() {
  ...
  async function onSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    // if (!form.restaurant_name.trim() || !form.restaurant_slug.trim()) {
    //   setErr("Restaurant name and slug are required");
    //   return;
    // }
    ...
    try {
      ...
      const msg =
        ...
        apiErr?.password?.[0] ||
        apiErr?.password2?.[0] ||
        apiErr?.restaurant_name?.[0] ||
        apiErr?.restaurant_slug?.[0] ||
        apiErr?.first_name?.[0] ||
        apiErr?.last_name?.[0] ||
