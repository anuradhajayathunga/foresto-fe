...
const menuItems = [
  {
    ...
    description:
      "Crispy fried chicken patty with our signature spicy mayo and jalapeños.",
    recipe: [
    ...
    <div className="min-h-screen bg-slate-50/50 p-6 md:p-8">
      ...
      <div className="max-w-[1600px] mx-auto space-y-6">
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
          <div>
            ...
            <p className="text-slate-500 text-sm mt-1">
              Manage your 24 active items and recipes.
            </p>
          </div>
        ...
        <div className="flex flex-col sm:flex-row gap-3">
          ...
          <Button
            variant="outline"
            className="bg-white text-slate-600 border-slate-200 border-dashed"
          >
          ...
          <Table>
            <TableHeader>
              <TableRow className="bg-slate-50/50 hover:bg-slate-50/50">
                ...
                <TableHead className="text-center font-medium text-slate-500">
                  Status
                </TableHead>
                <TableHead className="w-[50px]"></TableHead>
              </TableRow>
                ...
                return (
                  ...
                  >
                    ...
                    <TableCell>
                      <Badge
                        variant="secondary"
                        className="font-normal text-xs text-slate-600 bg-slate-100 border border-slate-200 hover:bg-slate-200"
                      >
                        {item.category}
                      </Badge>
                    </TableCell>
                    ...
                    <TableCell className="text-right">
                      <div className="flex justify-end items-center gap-1.5">
                        <span
                          className={`text-xs font-semibold tabular-nums ${isHighMargin ? "text-emerald-600" : "text-amber-600"}`}
                        >
                          {margin.toFixed(0)}%
                        </span>
        ...
        <SheetContent className="sm:max-w-[600px] w-full p-0 gap-0 overflow-hidden flex flex-col bg-white">
          ...
          <div className="px-6 py-6 border-b border-slate-100 bg-slate-50/50">
            ...
            <div className="grid grid-cols-3 gap-3">
              ...
              <Card
                className={`shadow-none border ${(selectedItem.price - selectedItem.cost) / selectedItem.price > 0.6 ? "bg-emerald-50/50 border-emerald-100" : "bg-amber-50/50 border-amber-100"}`}
              >
                <CardContent className="p-3">
                  <div
                    className={`text-[10px] uppercase tracking-wider font-semibold mb-1 ${(selectedItem.price - selectedItem.cost) / selectedItem.price > 0.6 ? "text-emerald-600" : "text-amber-600"}`}
                  >
                    Margin
                  </div>
                  <div
                    className={`text-lg font-bold tabular-nums ${(selectedItem.price - selectedItem.cost) / selectedItem.price > 0.6 ? "text-emerald-700" : "text-amber-700"}`}
                  >
                    {(
                    ...
                    <Table>
                      <TableHeader>
                        <TableRow className="bg-slate-50 hover:bg-slate-50">
                          ...
                          <TableHead className="text-xs font-semibold uppercase text-slate-500 text-right">
                            Qty
                          </TableHead>
                          <TableHead className="text-xs font-semibold uppercase text-slate-500 text-right">
                            Cost
