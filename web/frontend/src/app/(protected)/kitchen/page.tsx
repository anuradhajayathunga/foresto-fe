...
export default function KitchenPage() {
  ...
  const [alertInvoiceDate, setAlertInvoiceDate] = useState<string>(() => todayISO());
  const [alertInvoiceNo, setAlertInvoiceNo] = useState<string>('');
  const [alertNote, setAlertNote] = useState<string>('Auto-created from kitchen low-stock check');
  ...
  useEffect(() => { loadKitchenData(); }, [dateFrom, dateTo]);
  ...
  async function handleSaveProduction() {
    ...
    } catch (e: any) { setErr(parseError(e)); } finally { setSaving(false); }
  }
  ...
  async function handleForecastSuggest() {
    setSaving(true); setErr(null); setSuccess(null);
    try {
      const resp = await forecastSuggestProductions({ date: forecastDate, save_to_production: forecastSave === 'yes' });
  ...
  async function handleCheckSinglePlanAlert() {
    ...
    setSaving(true); setErr(null); setSuccess(null);
    try {
      const resp = await checkPlanAlerts({ date: prodDate, rows: [{ menu_item_id: Number(prodMenuItem), planned_qty: plannedQty || '0' }] });
      setPlanAlerts(resp.alerts);
      const alertCount = resp.alerts?.ingredient_alerts?.length || 0;
      setSuccess(alertCount > 0 ? `${alertCount} low-stock alert(s) detected.` : 'No low-stock alerts.');
    } catch (e: any) { setErr(parseError(e)); } finally { setSaving(false); }
  }
  ...
  async function handleCreatePurchaseRequestFromPlan() {
    ...
    try {
      const resp = await checkPlanAlerts({
        date: prodDate,
        rows: [{ menu_item_id: Number(prodMenuItem), planned_qty: plannedQty || '0' }],
      ...
      if (requestId) {
        ...
        setSuccess(`Purchase request #${requestId} created (${lineCount} line${lineCount === 1 ? '' : 's'}).`);
        await loadKitchenData();
      } else {
        setSuccess('No low-stock alerts found. Purchase request was not created.');
      }
    } catch (e: any) { setErr(parseError(e)); } finally { setSaving(false); }
  }
  ...
  async function handleCreatePurchaseDraftFromPlan() {
    ...
    try {
      ...
      if (invoiceId) {
        setSuccess(`Draft purchase invoice #${invoiceId} created from low-stock alerts.`);
        await loadKitchenData();
  ...
  function openConvertDialog(requestId: number) {
    ...
    if (suppliers.length && !convertSupplier) setConvertSupplier(String(suppliers[0].id));
    setConvertOpen(true);
  }
  ...
  async function handleConvertRequest() {
    ...
    try {
      ...
      setSuccess(`Converted request #${selectedRequestId} to invoice #${invoice.id}.`);
      await loadKitchenData();
  ...
  const selectedMenuItemName = items.find((item) => String(item.id) === prodMenuItem)?.name || '-';
    ...
    <div className='min-h-screen w-full bg-muted/30 p-6 md:p-8 space-y-8'>
      ...
      <div className='flex flex-col gap-6 lg:flex-row lg:items-center lg:justify-between'>
        <div className='space-y-1'>
          <h1 className='text-3xl font-bold tracking-tight text-foreground'>Kitchen Operations</h1>
          <p className='text-muted-foreground'>
            Manage production planning, track waste, and handle purchase requests.
          </p>
        </div>
      ...
      {/* 2. Notifications */}
      {(err || success) && (
        <div className={`rounded-lg border px-4 py-3 text-sm flex items-center gap-3 animate-in fade-in slide-in-from-top-2 ${
          err 
          ...
          {err ? <AlertTriangle className='h-4 w-4 shrink-0' /> : <ChefHat className='h-4 w-4 shrink-0' />}
      ...
      <div className='grid grid-cols-1 md:grid-cols-3 gap-6'>
        <Card className="border-l-4 border-l-blue-500 shadow-sm">
          <CardHeader className='flex flex-row items-center justify-between pb-2'>
            <CardTitle className='text-sm font-medium text-muted-foreground uppercase tracking-wider'>Production</CardTitle>
            <ChefHat className='h-4 w-4 text-muted-foreground' />
          </CardHeader>
          <CardContent>
            <div className='text-2xl font-bold'>{prodMetrics.count} <span className="text-sm font-normal text-muted-foreground">Items</span></div>
            <div className='mt-2 flex items-center gap-4 text-xs text-muted-foreground'>
              <div className="flex gap-1">
                <span className="font-medium text-foreground">{fmtQty(prodMetrics.totalPlanned)}</span> Planned
              </div>
              <Separator orientation="vertical" className="h-3" />
              <div className="flex gap-1">
                <span className="font-medium text-foreground">{fmtQty(prodMetrics.totalPrepared)}</span> Prepared
              </div>
            </div>
          </CardContent>
        </Card>
        ...
        <Card className="border-l-4 border-l-orange-500 shadow-sm">
          <CardHeader className='flex flex-row items-center justify-between pb-2'>
            <CardTitle className='text-sm font-medium text-muted-foreground uppercase tracking-wider'>Total Waste</CardTitle>
            <Trash2 className='h-4 w-4 text-muted-foreground' />
          </CardHeader>
          <CardContent>
            <div className='text-2xl font-bold'>{fmtQty(wasteMetrics.totalWaste)} <span className="text-sm font-normal text-muted-foreground">Units</span></div>
        ...
        <Card className="border-l-4 border-l-purple-500 shadow-sm">
          ...
          <CardContent>
            <div className='text-2xl font-bold'>{purchaseRequests.filter(r => r.status === 'DRAFT').length} <span className="text-sm font-normal text-muted-foreground">Drafts</span></div>
            <p className='text-xs text-muted-foreground mt-2'>
      ...
      <Tabs defaultValue='production' className='w-full space-y-6'>
        <div className="flex items-center justify-between">
            <TabsList className='grid grid-cols-3 w-full max-w-md bg-muted/50 p-1'>
              <TabsTrigger value='production' className="data-[state=active]:bg-background data-[state=active]:shadow-sm gap-2">
                <ChefHat className="h-4 w-4" /> Production
              </TabsTrigger>
              <TabsTrigger value='waste' className="data-[state=active]:bg-background data-[state=active]:shadow-sm gap-2">
                <Trash2 className="h-4 w-4" /> Waste
              </TabsTrigger>
              <TabsTrigger value='requests' className="data-[state=active]:bg-background data-[state=active]:shadow-sm gap-2">
        ...
        {/* --- Tab: Production --- */}
        <TabsContent value='production' className='space-y-6 animate-in fade-in slide-in-from-bottom-2'>
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <Card className="lg:col-span-1 h-fit shadow-md border-muted/60">
              <CardHeader className="bg-muted/20 border-b pb-4">
                ...
                <CardDescription>Plan or log prepared quantities.</CardDescription>
              </CardHeader>
              <CardContent className="space-y-4 pt-6">
                ...
                <div className='grid gap-1.5'>
                  <Label className="text-xs uppercase text-muted-foreground">Menu Item</Label>
                ...
                <div className="grid grid-cols-2 gap-4">
                    <div className='grid gap-1.5'>
                    <Label className="text-xs uppercase text-muted-foreground">Planned</Label>
                    <Input value={plannedQty} type='number' min='0' step='0.01' onChange={(e) => setPlannedQty(e.target.value)} />
                    </div>
                    <div className='grid gap-1.5'>
                    <Label className="text-xs uppercase text-muted-foreground">Prepared</Label>
                    <Input value={preparedQty} type='number' min='0' step='0.01' onChange={(e) => setPreparedQty(e.target.value)} />
                    </div>
                </div>

                <div className='grid gap-1.5'>
                  <Label className="text-xs uppercase text-muted-foreground">Note</Label>
                  <Textarea value={prodNote} onChange={(e) => setProdNote(e.target.value)} placeholder='Details...' className="resize-none h-20" />
                </div>

                <div className="rounded-lg border bg-muted/20 p-3 space-y-3">
                  <div className="flex items-center justify-between gap-3">
                    <div>
                      <p className="text-xs uppercase tracking-wide text-muted-foreground">Low-Stock Ingredient Check</p>
                      <p className="text-xs text-muted-foreground">Item: <span className="font-medium text-foreground">{selectedMenuItemName}</span></p>
                    </div>
                    <Badge variant="secondary">{ingredientAlerts.length} alert{ingredientAlerts.length === 1 ? '' : 's'}</Badge>
                  </div>
                  ...
                  {ingredientAlerts.length > 0 ? (
                    <div className="max-h-52 overflow-auto rounded-md border bg-background">
                      <Table>
                        <TableHeader>
                          <TableRow>
                            <TableHead className="h-8 text-xs">Ingredient</TableHead>
                            <TableHead className="h-8 text-xs text-right">Need</TableHead>
                            <TableHead className="h-8 text-xs text-right">Stock</TableHead>
                            <TableHead className="h-8 text-xs text-right">Suggested Buy</TableHead>
                          </TableRow>
                        </TableHeader>
                        <TableBody>
                            ...
                            <TableRow key={alert.item_id}>
                              <TableCell className="text-xs">
                                <div className="flex items-center gap-2">
                                  ...
                                  <Badge
                                    variant="outline"
                                    className={alert.severity === 'CRITICAL' ? 'border-red-300 text-red-700' : 'border-yellow-300 text-yellow-700'}
                                  >
                                    {alert.severity}
                                  </Badge>
                                </div>
                              </TableCell>
                              <TableCell className="text-xs text-right">{fmtQty(alert.required_qty)} {alert.unit || ''}</TableCell>
                              <TableCell className="text-xs text-right">{fmtQty(alert.current_stock)}</TableCell>
                              <TableCell className="text-xs text-right font-medium">{fmtQty(alert.suggested_purchase_qty)}</TableCell>
                            </TableRow>
                          ))}
                        </TableBody>
                      </Table>
                    ...
                    <p className="text-xs text-muted-foreground">Run <span className="font-medium">Check Alerts</span> to see ingredient shortages and suggested purchase quantities.</p>
                  )}

                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                    <div className='grid gap-1.5'>
                      <Label className="text-xs uppercase text-muted-foreground">Supplier</Label>
                      <Select value={alertSupplier} onValueChange={setAlertSupplier}>
                    ...
                    <div className='grid gap-1.5'>
                      <Label className="text-xs uppercase text-muted-foreground">Draft Date</Label>
                      <Input type='date' className="h-8" value={alertInvoiceDate} onChange={(e) => setAlertInvoiceDate(e.target.value)} />
                    </div>
                  </div>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                    <div className='grid gap-1.5'>
                      ...
                      <Input className="h-8" value={alertInvoiceNo} onChange={(e) => setAlertInvoiceNo(e.target.value)} placeholder='Optional' />
                    </div>
                    <div className='grid gap-1.5'>
                      <Label className="text-xs uppercase text-muted-foreground">Request Note</Label>
                      <Input className="h-8" value={alertNote} onChange={(e) => setAlertNote(e.target.value)} placeholder='Optional note' />
                    </div>
                  </div>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                    <Button
                      ...
                      disabled={saving || !prodMenuItem || Number(plannedQty || 0) <= 0 || ingredientAlerts.length === 0}
                    >
                    ...
                    <Button
                      ...
                      disabled={saving || !alertSupplier || !prodMenuItem || Number(plannedQty || 0) <= 0 || ingredientAlerts.length === 0}
                    >
              ...
              <CardFooter className="flex flex-col gap-2 bg-muted/10 border-t pt-4">
                <Button className="w-full" onClick={handleSaveProduction} disabled={saving}>
                   {saving && <RefreshCw className="mr-2 h-4 w-4 animate-spin" />}
                   {selectedProductionId ? 'Update Entry' : 'Add Entry'}
                </Button>
                <div className="grid grid-cols-2 gap-2 w-full">
                    <Button variant='outline' size="sm" onClick={handleCheckSinglePlanAlert} disabled={saving}>Check Alerts</Button>
                    <Button variant='ghost' size="sm" onClick={resetProductionForm} disabled={saving}>Clear</Button>
                </div>
              </CardFooter>
            </Card>
            ...
            <div className="lg:col-span-2 space-y-6">
                ...
                <div className="bg-gradient-to-r from-blue-50 to-indigo-50 dark:from-blue-950/30 dark:to-indigo-950/30 border border-blue-100 dark:border-blue-900 rounded-lg p-4 flex flex-col md:flex-row items-center justify-between gap-4">
                    <div className="flex items-center gap-3">
                        <div className="p-2 bg-blue-100 text-blue-700 rounded-full"><TrendingUp className="h-5 w-5" /></div>
                        <div>
                            <h4 className="font-semibold text-blue-900 dark:text-blue-200">AI Forecasting</h4>
                            <p className="text-xs text-blue-700 dark:text-blue-300">Generate suggested production quantities based on historical trends.</p>
                        </div>
                    </div>
                    <div className="flex items-center gap-2">
                        <Input type='date' className="h-8 w-36 bg-white" value={forecastDate} onChange={(e) => setForecastDate(e.target.value)} />
                        <Button size="sm" variant="secondary" className="bg-white hover:bg-white/90 text-blue-700 shadow-sm" onClick={handleForecastSuggest} disabled={saving}>
                            Run Forecast
                        </Button>
                    </div>
                </div>
                ...
                <Card className="shadow-sm">
                    ...
                    <Table>
                        ...
                        {productions.map((row) => (
                            ...
                            <TableCell className='text-right text-muted-foreground'>{fmtQty(row.suggested_qty)}</TableCell>
                            <TableCell className='text-right font-medium'>{fmtQty(row.planned_qty)}</TableCell>
                            <TableCell className='text-right font-medium'>{fmtQty(row.prepared_qty)}</TableCell>
                            </TableRow>
                        ))}
                        {!productions.length && (
                            <TableRow>
                            <TableCell colSpan={5} className='text-center h-32 text-muted-foreground'>No production records found for this period.</TableCell>
                            </TableRow>
                        )}
                        </TableBody>
                    </Table>
        ...
        {/* --- Tab: Waste --- */}
        <TabsContent value='waste' className='space-y-6 animate-in fade-in slide-in-from-bottom-2'>
                    ...
                    <CardContent className="space-y-4 pt-6">
                        ...
                        <div className="grid grid-cols-2 gap-4">
                            <div className='grid gap-1.5'>
                                ...
                                <Input value={wasteQty} type='number' min='0' step='0.01' onChange={(e) => setWasteQty(e.target.value)} placeholder='Auto' />
                            </div>
                            <div className='grid gap-1.5'>
                        ...
                        <div className='grid gap-1.5'>
                            <Label className="text-xs uppercase text-muted-foreground">Note</Label>
                            <Textarea value={wasteNote} onChange={(e) => setWasteNote(e.target.value)} placeholder='Optional...' className="resize-none h-20" />
                        </div>
                        <Button className="w-full" onClick={handleSaveWaste} disabled={saving}>Record Waste</Button>
                    </CardContent>
                </Card>
                ...
                <div className="lg:col-span-8 space-y-6">
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                         <Card className="bg-muted/20 border-dashed">
                             <CardContent className="p-4 flex flex-col items-center justify-center text-center">
                                 <span className="text-xs font-semibold uppercase text-muted-foreground">Total Waste</span>
                                 <span className="text-xl font-bold mt-1">{fmtQty(wasteSummary?.total_waste)}</span>
                             </CardContent>
                         </Card>
                         {(wasteSummary?.by_reason || []).map((r) => (
                             <Card key={r.reason} className="bg-white">
                                <CardContent className="p-4 flex flex-col items-center justify-center text-center">
                                    <span className="text-xs font-semibold uppercase text-muted-foreground">{r.reason}</span>
                                    <span className="text-xl font-bold mt-1">{fmtQty(r.total_waste)}</span>
                                </CardContent>
                            </Card>
                         ))}
                    </div>
                            ...
                            <Table>
                                ...
                                <TableBody>
                                    ...
                                    <TableRow key={row.id}>
                                        ...
                                        <TableCell className='text-right font-medium'>{fmtQty(row.waste_qty)}</TableCell>
                                    </TableRow>
                                    ))}
                                    {!wastes.length && <TableRow><TableCell colSpan={4} className='text-center h-20 text-muted-foreground'>No waste recorded.</TableCell></TableRow>}
                                </TableBody>
                            </Table>
                        </CardContent>
                    </Card>
                    ...
                    <div className="border rounded-lg bg-white p-4">
                         <div className="flex items-center justify-between mb-4">
                             <div>
                                <h3 className="font-semibold text-sm">Waste vs. Sales Analysis</h3>
                                <p className="text-xs text-muted-foreground">Identify high-waste items.</p>
                             </div>
                             <Button size="sm" variant="outline" onClick={handleLoadWasteVsSales}>Load Analysis</Button>
                         </div>
                         {wasteVsSales.length > 0 && (
                             <Table>
                                ...
                                <TableRow className="bg-muted/20">
                                    ...
                                    <TableHead className="h-8 text-xs text-right">Sold</TableHead>
                                    <TableHead className="h-8 text-xs text-right">Waste</TableHead>
                                    <TableHead className="h-8 text-xs text-right">Waste %</TableHead>
                                </TableRow>
                                </TableHeader>
                                <TableBody>
                                {wasteVsSales.map((row) => (
                                    <TableRow key={row.menu_item_id} className="hover:bg-transparent">
                                    <TableCell className="py-2 text-sm">{row.menu_item_name}</TableCell>
                                    <TableCell className="py-2 text-sm text-right text-muted-foreground">{fmtQty(row.sold_qty)}</TableCell>
                                    <TableCell className="py-2 text-sm text-right text-red-600 font-medium">{fmtQty(row.waste_qty)}</TableCell>
                                    <TableCell className="py-2 text-sm text-right">{row.waste_rate_pct.toFixed(1)}%</TableCell>
                                    </TableRow>
                                ))}
                                </TableBody>
                            </Table>
           ...
           <Card>
            <CardHeader className="py-4 px-6 border-b">
                <CardTitle className="text-base flex items-center gap-2">
                    <ShoppingCart className="w-4 h-4 text-primary" /> Purchase Requests
                </CardTitle>
                <CardDescription>Automated requests generated from inventory shortages.</CardDescription>
            </CardHeader>
            <CardContent className="p-0">
                <Table>
                  <TableHeader>
                  ...
                  <TableBody>
                    {purchaseRequests.map((req) => (
                      <TableRow key={req.id}>
                        ...
                        <TableCell className="text-muted-foreground text-sm">{req.source_plan_date || '-'}</TableCell>
                        <TableCell>
                          <Badge 
                            variant="secondary" 
                            className={
                                req.status === 'DRAFT' ? 'bg-yellow-100 text-yellow-800 hover:bg-yellow-100' :
                                req.status === 'CONVERTED' ? 'bg-green-100 text-green-800 hover:bg-green-100' : 
                        ...
                        <TableCell className='text-right'>
                          <div className="flex justify-end items-center gap-2">
                            <Button size='sm' variant='ghost' className="h-8 text-xs" asChild>
                                <Link href={`/kitchen/requests/${req.id}`}>Details</Link>
                            </Button>
                            {req.status === 'DRAFT' && (
                                <>
                                <Separator orientation="vertical" className="h-4" />
                                <Button size='sm' variant='outline' className="h-8 text-xs border-green-200 hover:bg-green-50 text-green-700" onClick={() => handleSubmitRequest(req.id)} disabled={saving}>
                                    Submit
                                </Button>
                                <Button size='sm' className="h-8 text-xs" onClick={() => openConvertDialog(req.id)} disabled={saving}>
                    ...
                    {!purchaseRequests.length && (
                      <TableRow>
                        <TableCell colSpan={6} className='text-center h-32 text-muted-foreground'>No requests found.</TableCell>
                      </TableRow>
                    )}
                  </TableBody>
                </Table>
        ...
        <DialogContent className="sm:max-w-md">
          ...
          <div className='grid gap-4 py-2'>
            ...
            <div className="grid grid-cols-2 gap-4">
                ...
                <div className='grid gap-1.5'>
                    <Label className="text-xs uppercase text-muted-foreground">Invoice No</Label>
                    <Input value={convertInvoiceNo} onChange={(e) => setConvertInvoiceNo(e.target.value)} placeholder='Optional' />
                </div>
            </div>
            <div className='grid gap-1.5'>
              <Label className="text-xs uppercase text-muted-foreground">Note</Label>
              <Textarea value={convertNote} onChange={(e) => setConvertNote(e.target.value)} className="resize-none h-20" placeholder='Add a note to the invoice...' />
            </div>
          </div>

          <DialogFooter className="gap-2 sm:gap-0">
            <Button variant='outline' onClick={() => setConvertOpen(false)}>Cancel</Button>
            <Button onClick={handleConvertRequest} disabled={saving || !convertSupplier}>Confirm Conversion</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
    </div>
