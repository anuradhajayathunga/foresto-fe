...
export default function KitchenRequestDetailPage() {
  ...
  const [requestData, setRequestData] = useState<KitchenPurchaseRequest | null>(
    null,
  );
  ...
  const totalSuggested = useMemo(() => {
    return (requestData?.lines || []).reduce(
      (acc, line) => acc + Number(line.suggested_purchase_qty || 0),
      0,
    );
  }, [requestData]);
  ...
  const canConvert =
    requestData.status !== "CANCELLED" && requestData.status !== "CONVERTED";
  const shownStatus = displayRequestStatus(requestData.status);
    ...
    <div className="flex flex-col gap-6 mx-auto w-full max-w-6xl">
      <div className="flex items-center justify-between gap-3">
        <div className="space-y-1">
          ...
          <h1 className="text-3xl font-bold tracking-tight text-foreground">
            Kitchen Request #{requestData.id}
          </h1>
          <p className="text-sm text-muted-foreground">
            Review line items and convert to purchase draft invoice.
          </p>
        </div>
        <Badge variant={shownStatus === "DRAFT" ? "secondary" : "outline"}>
          {shownStatus}
        </Badge>
      </div>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Ingredients required from low-stock planning alerts.
          </CardDescription>
        </CardHeader>
        <CardContent>
                ...
                {(requestData.lines || []).map((line) => (
                  <TableRow key={line.id}>
                    ...
                    <TableCell className="text-right">
                      {fmtQty(line.required_qty)}
                    </TableCell>
                    <TableCell className="text-right">
                      {fmtQty(line.current_stock)}
                    </TableCell>
                    <TableCell className="text-right">
                      {fmtQty(line.reorder_level)}
                    </TableCell>
                    <TableCell className="text-right">
                      {fmtQty(line.suggested_purchase_qty)}
                    </TableCell>
      ...
      <Card>
        <CardHeader>
          ...
          {requestData.note && (
            <CardDescription>{requestData.note}</CardDescription>
          )}
        </CardHeader>
      ...
      <Dialog open={convertOpen} onOpenChange={setConvertOpen}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Create a draft purchase invoice from this kitchen request.
            </DialogDescription>
          </DialogHeader>
          ...
          <DialogFooter>
            ...
            <Button onClick={handleConvert} disabled={saving || !supplierId}>
              Convert
            </Button>
          </DialogFooter>
        </DialogContent>
