...
function calcAccuracy(referenceQty: number | null, actualQty: number): number | null {
...
function percentBadge(value: number | null) {
  ...
  if (value >= 70) {
    return <Badge className="bg-amber-50 text-amber-700 ring-1 ring-inset ring-amber-600/20 hover:bg-amber-50 font-medium">{pct(value)}</Badge>;
  }
  return <Badge className="bg-rose-50 text-rose-700 ring-1 ring-inset ring-rose-600/20 hover:bg-rose-50 font-medium">{pct(value)}</Badge>;
}
...
function buildDashboardData(
  ...
  function ensureRow(date: string, menuItemId: number, menuItemName: string): RowAccumulator {
    const key = `${date}__${menuItemId}`;
    const existing = rowMap.get(key);
    if (existing) return existing;

    const row: RowAccumulator = {
      key, date, menu_item_id: menuItemId, menu_item_name: menuItemName || `Item #${menuItemId}`,
      forecast_qty: null, planned_qty: null, prepared_qty: null, sold_qty: 0, waste_qty: 0,
      production_row_count: 0, sales_line_count: 0, waste_entry_count: 0,
    };
    rowMap.set(key, row);
    return row;
  }
  ...
  for (const sale of sales) {
    ...
    for (const item of sale.items ?? []) {
      ...
      const row = ensureRow(saleDate, menuItemId, item.menu_item_name ?? item.name ?? "");
  ...
  const rows: ForecastAccuracyRow[] = Array.from(rowMap.values())
    .map((r) => {
      ...
      const sellThrough = r.prepared_qty != null && r.prepared_qty > 0 ? (r.sold_qty / r.prepared_qty) * 100 : null;
      ...
      return {
        ...r, actual_demand_qty: actualDemand, forecast_accuracy_pct: forecastAccuracy,
        plan_accuracy_pct: planAccuracy, sell_through_pct: sellThrough, over_prep_qty: overPrep, under_prep_qty: underPrep,
      };
    })
    .sort((a, b) => a.date === b.date ? a.menu_item_name.localeCompare(b.menu_item_name) : a.date.localeCompare(b.date));

  const rowCount = rows.length;
  const forecastCoverageCount = rows.filter((r) => r.forecast_qty != null).length;

  const accuracyByDate = new Map<string, { f: Array<number | null>; p: Array<number | null> }>();
  ...
  const accuracyTrend = Array.from(accuracyByDate.entries())
    .map(([date, vals]) => ({ date, forecastAccuracy: avg(vals.f), planAccuracy: avg(vals.p) }))
    .sort((a, b) => a.date.localeCompare(b.date));

  const worstForecastRows = rows
    .filter((r) => r.forecast_qty != null)
    .map((r) => ({ label: `${r.date} - ${r.menu_item_name}`, error: Math.abs((r.forecast_qty ?? 0) - r.actual_demand_qty) }))
    .sort((a, b) => b.error - a.error).slice(0, 10);

  const worstPlanRows = rows
    .filter((r) => r.planned_qty != null)
    .map((r) => ({ label: `${r.date} - ${r.menu_item_name}`, error: Math.abs((r.planned_qty ?? 0) - r.actual_demand_qty) }))
    .sort((a, b) => b.error - a.error).slice(0, 10);

  return {
    rows,
    kpis: {
      rowCount, forecastCoverageCount,
      forecastCoveragePct: rowCount > 0 ? (forecastCoverageCount / rowCount) * 100 : null,
...
export default function ForecastAccuracyPage() {
  ...
  const [dashboard, setDashboard] = useState<ForecastAccuracyData>({
    rows: [],
    kpis: {
      rowCount: 0, forecastCoverageCount: 0, forecastCoveragePct: null,
      avgForecastAccuracyPct: null, avgPlanAccuracyPct: null, avgSellThroughPct: null,
      totalActualDemandQty: 0, totalSoldQty: 0, totalWasteQty: 0, totalOverPrepQty: 0, totalUnderPrepQty: 0,
    },
    charts: { accuracyTrend: [], worstForecastRows: [], worstPlanRows: [] },
  });
  ...
  async function loadData() {
    ...
    setError(null);
    try {
      const [productions, wastes, sales] = await Promise.all([
        listProductions({ date_from: dateFrom, date_to: dateTo }),
        listWastes({ date_from: dateFrom, date_to: dateTo }),
        listSales(),
      ]);
      setDashboard(buildDashboardData(productions, wastes, sales, dateFrom, dateTo));
    } catch (e) {
  ...
  const filteredRows = useMemo(() => {
    ...
    if (coverageFilter === "with_forecast") rows = rows.filter((r) => r.forecast_qty != null);
    else if (coverageFilter === "no_forecast") rows = rows.filter((r) => r.forecast_qty == null);

    if (search.trim()) {
      const q = search.trim().toLowerCase();
      rows = rows.filter((r) => r.menu_item_name.toLowerCase().includes(q) || r.date.includes(q));
    }
    return rows;
            ...
            {trendData.length === 0 ? (
              ...
              <ResponsiveContainer width="100%" height="100%">
                <LineChart data={trendData} margin={{ top: 5, right: 10, left: -20, bottom: 0 }}>
                  ...
                  <YAxis domain={[0, 100]} tickLine={false} axisLine={false} tickMargin={12} fontSize={11} tick={{ fill: '#64748b' }} />
                  <Tooltip content={<CustomChartTooltip />} cursor={{ stroke: '#cbd5e1', strokeWidth: 1, strokeDasharray: '4 4' }} />
                  <Legend iconType="circle" wrapperStyle={{ fontSize: '12px', paddingTop: '10px' }} />
                  <Line type="monotone" dataKey="forecastAccuracy" name="Forecast %" stroke="#6366f1" strokeWidth={3} dot={false} activeDot={{ r: 5 }} />
                  <Line type="monotone" dataKey="planAccuracy" name="Plan %" stroke="#10b981" strokeWidth={3} dot={false} activeDot={{ r: 5 }} />
                </LineChart>
              </ResponsiveContainer>
            )}
          </CardContent>
        ...
        <Card className="shadow-sm border-slate-200/60">
          ...
          <CardContent className="pt-6">
            {dashboard.charts.worstForecastRows.length === 0 ? (
              <div className="flex items-center justify-center h-[280px] text-sm text-slate-400">No forecast data available.</div>
            ) : (
              <div className="h-[280px]">
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart data={dashboard.charts.worstForecastRows} layout="vertical" margin={{ left: 10, right: 20 }}>
                    ...
                    <YAxis type="category" dataKey="label" width={180} tickLine={false} axisLine={false} tick={{ fontSize: 11, fill: '#475569' }} />
                    <Tooltip cursor={{ fill: '#f1f5f9' }} contentStyle={{ borderRadius: '8px', border: '1px solid #e2e8f0', fontSize: '12px' }} />
                    <Bar dataKey="error" name="Absolute Error" fill="#f43f5e" radius={[0, 4, 4, 0]} barSize={20} />
                  </BarChart>
                </ResponsiveContainer>
              </div>
            )}
        ...
        <Card className="shadow-sm border-slate-200/60">
          ...
          <CardContent className="pt-6">
            {dashboard.charts.worstPlanRows.length === 0 ? (
              <div className="flex items-center justify-center h-[280px] text-sm text-slate-400">No plan data available.</div>
            ) : (
              <div className="h-[280px]">
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart data={dashboard.charts.worstPlanRows} layout="vertical" margin={{ left: 10, right: 20 }}>
                    <CartesianGrid strokeDasharray="4 4" horizontal={false} stroke="#e2e8f0" />
                    <XAxis type="number" tickLine={false} axisLine={false} fontSize={11} tick={{ fill: '#64748b' }} />
                    <YAxis type="category" dataKey="label" width={180} tickLine={false} axisLine={false} tick={{ fontSize: 11, fill: '#475569' }} />
                    <Tooltip cursor={{ fill: '#f1f5f9' }} contentStyle={{ borderRadius: '8px', border: '1px solid #e2e8f0', fontSize: '12px' }} />
                    <Bar dataKey="error" name="Absolute Error" fill="#f59e0b" radius={[0, 4, 4, 0]} barSize={20} />
                  </BarChart>
                </ResponsiveContainer>
              </div>
            )}
              ...
              {loading ? (
                ...
                filteredRows.map((row) => (
                  <TableRow key={row.key} className="hover:bg-slate-50/50 transition-colors group border-b border-slate-100">
                    ...
                    <TableCell className="text-right tabular-nums text-sm text-slate-600">{fmt(row.forecast_qty)}</TableCell>
                    <TableCell className="text-right tabular-nums text-sm text-slate-600">{fmt(row.planned_qty)}</TableCell>
