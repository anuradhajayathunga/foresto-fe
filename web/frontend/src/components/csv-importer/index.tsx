...
export function CsvImporter({ kind, title, description }: CsvImporterProps) {
  ...
  const recipeColumnsHint =
    "Required recipe columns: menu_item_slug, menu_category_slug, ingredient_sku, qty.";
  ...
  async function downloadTemplate() {
    ...
    try {
      ...
      setErr(e?.detail || e?.message || "Could not download template.");
    } finally {
    ...
    <div className="w-full space-y-4">
      ...
      <div className=" p-4 space-y-4">
        ...
        <div className="space-y-2">
          ...
          {kind === "recipes" ? (
            <p className="text-xs text-muted-foreground">{recipeColumnsHint}</p>
          ) : null}
        </div>
