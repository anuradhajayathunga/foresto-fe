...
from purchases.services_email import (
    build_purchase_email_subject,
    get_purchase_email_template_meta,
)
...
class EmailServiceTests(SimpleTestCase):
    def test_template_for_request_status(self):
        title, action = get_purchase_email_template_meta(PurchaseInvoice.Status.REQUEST)
        self.assertEqual(title, "Purchase Order Request")
        self.assertIn("confirm item availability", action.lower())

    def test_subject_uses_invoice_id_and_status_title(self):
        invoice = SimpleNamespace(id=42, status=PurchaseInvoice.Status.CONFIRMED)
        subject = build_purchase_email_subject(invoice)
        self.assertEqual(subject, "Purchase Order Confirmed - PO #42")
